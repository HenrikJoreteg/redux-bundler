...
test('reactorsBundle', (t) => {
  ...
  t.deepEqual(store.getState().reactionary, {started: true, a0: false, a1: false})
  t.equal(count, 1)
  setTimeout(() => {
    t.equal(count, 2, 'should now have ran twice')
    t.deepEqual(store.getState().reactionary, {started: true, a0: true, a1: false}, 'more state changes should have occured')
    setTimeout(() => {
      t.equal(count, 3, 'should now have ran thrice')
      t.deepEqual(store.getState().reactionary, {started: true, a0: true, a1: true}, 'all state changes should have occured')
      t.end()
    }, 0)
  }, 0)
})
