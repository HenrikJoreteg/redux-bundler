...
const { composeBundles, composeBundlesRaw, asyncCountBundle, appTimeBundle } = require('../')
...
test('integrateBundles reactors', t => {
  ...
  t.equal(store.doThing, doThing, 'instance of bound action creator has not been replaced')
  t.equal(store.selectIsDone(), true, 'should still have kept state of original')
  t.equal(store.selectIsFlippedUp(), false, 'should have gotten initial state for new bundle added later')
  store.doFlipUp()
  ...
  t.equal(store.isReacting, true, 'store reacting flag should be true')
  ...
  setTimeout(() => {
    ...
    t.equal(store.isReacting, false, 'store reacting flag should be false')
    t.end()
  })
})
