...
test('bindings errors', (t) => {
  ...
  }, /SelectorNotFound/, 'should error if trying to subscribe to missing things')
  t.end()
})
...
test('bindings basic functionality', (t) => {
  ...
  store.subscribeToSelectors(['selectIsPositive', 'selectIsTrue'], (changes) => {
    t.deepEqual(changes, {isPositive: true}, 'there should only be this one change')
    t.end()
  })
  t.deepEqual(store.subscriptions.watchedValues, {isPositive: false, isTrue: true})
  store.dispatch({type: 'INCREMENT'})
  t.deepEqual(store.subscriptions.watchedValues, {isPositive: true, isTrue: true})
...
test('bindings select all functionality', (t) => {
  ...
  store.subscribeToAllChanges((changes) => {
    count++
    if (count === 1) {
      t.deepEqual(changes, {isPositive: true, number: 1}, 'first instance of all callback should have isPositive and number only')
    } else if (count === 2) {
  ...
  store.subscribeToSelectors(['selectIsPositive', 'selectNumber'], (changes) => {
    t.deepEqual(changes, {isPositive: true, number: 1}, 'there should only be this one change')
  })
  t.deepEqual(store.subscriptions.watchedValues, {isPositive: false, isTrue: true, number: 0})
  store.dispatch({type: 'INCREMENT'})
  t.deepEqual(store.subscriptions.watchedValues, {isPositive: true, isTrue: true, number: 1})
  t.equal(store.selectIsPositive(), true, 'now it is positive')
